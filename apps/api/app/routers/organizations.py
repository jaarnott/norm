...
from app.db.models import Organization, OrganizationMembership, UserVenueAccess, Venue, User, ConnectorConfig
...
async def list_organizations(db: Session = Depends(get_db), user: User = Depends(get_current_user)):
...
async def create_organization(body: CreateOrgBody, db: Session = Depends(get_db), user: User = Depends(require_role("admin"))):
...
async def update_organization(org_id: str, body: UpdateOrgBody, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
...
async def add_member(org_id: str, body: AddMemberBody, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
...
async def remove_member(org_id: str, member_user_id: str, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
...
async def create_venue(org_id: str, body: CreateVenueBody, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
...
async def update_venue(venue_id: str, body: UpdateVenueBody, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
...
async def delete_venue(venue_id: str, db: Session = Depends(get_db), user: User = Depends(require_role("admin"))):
...
async def list_venue_connectors(venue_id: str, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    """List connector configuration status for a specific venue."""
    venue = db.query(Venue).filter(Venue.id == venue_id).first()
    if not venue:
        raise HTTPException(404, "Venue not found")

    configs = db.query(ConnectorConfig).filter(ConnectorConfig.venue_id == venue_id).all()
    return {
        "venue_id": venue_id,
...
async def list_user_venues(user_id: str, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
...
async def set_user_venues(user_id: str, body: dict, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
...
async def get_usage(org_id: str, month: str | None = None, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
...
async def get_daily_usage(org_id: str, month: str | None = None, user_id: str | None = None, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    for r in rows:
        if r.date not in by_day:
            by_day[r.date] = {"input_tokens": 0, "output_tokens": 0, "llm_call_count": 0}
...
async def find_user_by_email(email: str, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    """Find a user by email address. Used for adding members to an organization."""
    target = db.query(User).filter(User.email == email).first()
    if not target:
        raise HTTPException(404, "User not found")
    return {"id": target.id, "email": target.email, "full_name": target.full_name, "role": target.role}
