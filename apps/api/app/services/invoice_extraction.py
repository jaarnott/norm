...
BUILTIN_MAIN_PROMPT = (
    ...
    "'2L'.\n"
    "- BEVERAGES — pick the unit by how the venue SELLS the item, "
    "which OVERRIDES 'keep exactly as printed':\n"
    "  · Poured or portioned by the glass, nip or measure (wine, "
    "spirits, kegs, cordials/syrups, juice used as a mixer) → a "
    "VOLUME unit: a case of wine 12x750ml → '12x750ml', a 700ml "
    "spirit → '700ml', a 50L keg → '50L'.\n"
    "  · Sold as a whole sealed EACH (bottled/canned beer, cider, "
    "RTDs/premixes, soft drinks, water) → a PACK OF EACHES: count "
    "the units and DROP the per-item volume — beer 12x330ml → "
    "'12 pack' (NEVER '12x330ml'), 24x330ml → '24 pack', a single "
    "bottle/can → 'each'.\n"
    ...
    "- NEVER INVENT a unit: unit_of_measure must be backed by a real "
    "size PRINTED on the line — a weight, a volume, or a pack of "
    "explicitly-sized items. Quantity or packaging notation is NOT a "
    "size: a bottle/can count ('NRB X1', a leading '1.0 ... X1'), a "
    "bare 'each'/'pack' with no per-item size, or an ABV% like '37.5' "
    "— so a spirit or wine printed with no millilitre size gives NO "
    "unit. When no real size is printed, return null (do NOT guess a "
    "count like 'each' or '1 pack') — the unit is worked out "
    "downstream.\n"
