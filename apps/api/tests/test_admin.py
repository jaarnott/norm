...
class TestListDeployments:
    ...
    def test_returns_deployments_after_webhook(self, client, admin_headers):
        ...
        client.post("/api/admin/deploy-webhook", json={
            "environment": "testing",
            "image_tag": "v1.0.0",
            "git_sha": "abc123",
            "status": "success",
...
class TestDeployWebhook:
    ...
    def test_creates_deployment_record(self, client):
        ...
        resp = client.post("/api/admin/deploy-webhook", json={
...
class TestListEnvironments:
    ...
    def test_environments_have_null_latest_deploy_initially(self, client, admin_headers):
        resp = client.get("/api/admin/environments", headers=admin_headers)
        envs = resp.json()["environments"]
        for env in envs:
