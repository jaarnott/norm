...
from app.db.models import Task, Order, OrderLine, Supplier, Product
...
class TestListOrders:
    ...
    def test_list_orders(self, client, db_session, admin_user, admin_headers):
        task = Task(
            ...
            extracted_fields={"product": {"id": "p1", "name": "Milk"}, "venue": {"id": "v1", "name": "HQ"}},
        )
        db_session.add(task)
        db_session.flush()
...
class TestApproveOrder:
    ...
    @patch("app.routers.orders.approve_order")
    def test_approve_order_not_found_returns_404(self, mock_approve, client, admin_headers):
...
class TestRejectOrder:
    ...
    @patch("app.routers.orders.reject_order")
    def test_reject_order_not_found_returns_404(self, mock_reject, client, admin_headers):
...
class TestSubmitOrder:
    ...
    def test_submit_order_not_found_or_not_approved_returns_404(
        self, mock_submit, client, admin_headers,
    ):
