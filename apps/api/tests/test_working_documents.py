...
from app.db.models import Task, WorkingDocument


class TestListDocuments:
    """GET /api/tasks/{task_id}/working-documents"""
    ...
    def test_list_documents(self, client, db_session, admin_user, admin_headers):
        ...
        resp = client.get(f"/api/tasks/{task.id}/working-documents", headers=admin_headers)
    ...
    def test_list_documents_without_auth_returns_401(self, client):
        resp = client.get(f"/api/tasks/{uuid.uuid4()}/working-documents")
        assert resp.status_code in (401, 403)


class TestGetDocument:
    """GET /api/tasks/{task_id}/working-documents/{doc_id}"""
    ...
    def test_get_document(self, client, db_session, admin_user, admin_headers):
        ...
        resp = client.get(
            f"/api/tasks/{task.id}/working-documents/{doc.id}",
    ...
    def test_get_document_not_found_returns_404(self, client, admin_headers):
        ...
        resp = client.get(
            f"/api/tasks/{task_id}/working-documents/{doc_id}",
            headers=admin_headers,
        )
...
class TestPatchDocument:
    """PATCH /api/tasks/{task_id}/working-documents/{doc_id}"""

    def test_patch_document_add_line(self, client, db_session, admin_user, admin_headers):
        task = Task(
            id=str(uuid.uuid4()),
            user_id=admin_user.id,
            domain="procurement",
        ...
        resp = client.patch(
            f"/api/tasks/{task.id}/working-documents/{doc.id}",
            json={
                "ops": [{"op": "add_line", "fields": {"product": "Milk", "quantity": 5}}],
                "version": 1,
            },
            headers=admin_headers,
        )
    ...
    def test_patch_document_not_found_returns_404(self, client, admin_headers):
        resp = client.patch(
            f"/api/tasks/{uuid.uuid4()}/working-documents/{uuid.uuid4()}",
            json={"ops": [], "version": 1},
            headers=admin_headers,
        )
        assert resp.status_code == 404
...
class TestGetStandaloneDocument:
    ...
    def test_get_standalone_document(self, client, db_session, admin_user, admin_headers):
        doc = WorkingDocument(
            id=str(uuid.uuid4()),
            task_id=None,
            doc_type="roster",
    ...
    def test_get_standalone_document_not_found_returns_404(self, client, admin_headers):
        resp = client.get(f"/api/working-documents/{uuid.uuid4()}", headers=admin_headers)
        assert resp.status_code == 404
...
class TestPatchStandaloneDocument:
    ...
    def test_patch_standalone_document(self, client, db_session, admin_user, admin_headers):
        doc = WorkingDocument(
            id=str(uuid.uuid4()),
            task_id=None,
            doc_type="order",
            connector_name="bidfood",
            sync_mode="submit",
            data={"lines": [{"product": "Bread", "quantity": 2, "unit": "case", "supplier": "", "unit_price": 0}]},
            version=1,
        )
        db_session.add(doc)
        db_session.flush()
