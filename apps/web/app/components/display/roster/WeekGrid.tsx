  ...
  return (
    <div
      ...
      style={{
        ...
        border: isSelected ? '1px solid var(--focus)' : '1px solid var(--line)',
        backgroundColor: isSelected ? 'var(--draft-bg)' : 'var(--bg)',
        ...
        {hrs > 0 && (() => {
          ...
          return (
            <div style={{ fontSize: '0.68rem', color: 'var(--muted)' }}>
              {hrs.toFixed(1)}h{breakMins > 0 && <span style={{ marginLeft: 3, color: 'var(--muted-soft)' }}>({Math.round(breakMins)}m brk)</span>}
            </div>
          );
        })()}
      </div>
  ...
  return (
    <div
      ...
      style={{
        ...
        minHeight: 40,
        backgroundColor: isOver ? 'var(--draft-bg)' : isToday ? 'var(--surface)' : undefined,
  ...
  return (
    <div style={{ overflowX: 'auto', border: '1px solid var(--line)', borderRadius: 8 }}>
      <div style={{ minWidth: 700, fontSize: '0.78rem' }}>
        {/* Header row */}
        <div style={{ display: 'grid', gridTemplateColumns: cols, borderBottom: '2px solid var(--line)', backgroundColor: 'var(--surface)' }}>
          <div style={{
            padding: '0.5rem 0.6rem', fontWeight: 600, color: 'var(--text-soft)',
            borderRight: '1px solid var(--line)',
            position: 'sticky', left: 0, zIndex: 1, backgroundColor: 'var(--surface)',
          }}>Staff</div>
          {days.map((d, i) => {
            ...
            return (
              <div
                ...
                style={{
                  ...
                  fontWeight: 600, color: isToday ? 'var(--focus)' : 'var(--text-soft)',
                  backgroundColor: isToday ? 'var(--draft-bg)' : 'var(--surface)',
                  whiteSpace: 'nowrap', cursor: 'pointer',
                }}
              >
                <div>{DAY_NAMES[d.getDay()]}</div>
                <div style={{ fontSize: '0.85em', fontWeight: 500, color: isToday ? 'var(--focus)' : 'var(--muted)' }}>{d.getDate()}</div>
              </div>
            );
          })}
        </div>
        ...
        {staffRows.map((row, ri) => (
          <div
            ...
            style={{
              ...
              backgroundColor: ri % 2 === 1 ? 'var(--surface)' : 'var(--bg)',
            }}
          >
            {/* Staff name cell */}
            <div style={{
              ...
              <div style={{ fontWeight: 500, color: 'var(--text)', lineHeight: 1.3 }}>{row.name}</div>
              <div style={{ fontSize: '0.7rem', color: 'var(--muted)' }}>{row.role}</div>
            </div>
